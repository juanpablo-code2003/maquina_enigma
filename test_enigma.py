...
class TestEnigma(unittest.TestCase):
  ...
  def test_engima_definido_encriptacion_prevista(self):
    mensaje = 'MENSAJE'
    configuracion = enigma.Config(aleatorio=False, cadena_inicial='AZY', cadena_caracteres_vuelta_completa='ZZZ', caracteres_permitidos=utils.ALFABETO_ENG)
    encriptado = enigma.enigma(msg=mensaje, config=configuracion)
    rotores_str = list()
    for r in configuracion.rotores:
      rotores_str.append(r.__str__())
    self.assertEqual(encriptado, 'XOROMCH', rotores_str)
